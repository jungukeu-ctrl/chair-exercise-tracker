...
## 현재 브랜치
- `claude/exercise-screen-firebase-removal-f0hag4` — 운동 화면 Firebase 제거(localStorage 전용 전환) 완료, PR #24로 main 병합됨.
- 다음 기능 작업 시작 시 origin/main 기준으로 새 브랜치를 판다.
...
## 남은 작업 (To-Do)
코드/스키마/규칙은 모두 작성 완료.
- [ ] 운동 카드 "▶ 영상 보기" 링크가 실사이트에서 보이고, 클릭 시 해당 동작설명 시점부터 유튜브가 재생되는지 사용자 최종 확인 필요 — 이 세션의 네트워크 정책이 `jungukeu-ctrl.github.io` 접속을 차단(403)해 직접 확인 불가 (배포 자체는 `npm run deploy` 완료)
...
| 2026-07-07 | Cloud Functions 배포는 Blaze 요금제 비용 부담으로 v1 스코프에서 제외하기로 확정(사용자 결정). `src/pages/Settings.jsx`의 알림 시간 설정 UI 위에 "알림 발송 기능은 현재 준비 중입니다" 안내 문구(`notif-notice`, `App.css`에 스타일 추가, 기존 `.device-notice`와 동일 톤) 추가 — 시간 설정 저장과 "이 기기에서 알림 받기"(FCM 토큰 등록) 버튼은 그대로 유지해 추후 Cloud Functions 배포 시 바로 쓸 수 있게 함. `functions/` 코드는 삭제하지 않고 보존. `npm run build` 통과. **배포 관련 예외**: 이 세션의 네트워크 정책이 `jungukeu-ctrl.github.io` 접속을 차단(403)해 배포 자체(`npm run build`+`npm run deploy`)는 완료했으나 실사이트 콘솔 확인은 직접 못 함 → 사용자가 실사이트에서 안내 문구 노출을 최종 확인(2026-07-07). 타 프로젝트 영향 없음 |
...
| 2026-07-18 | "아버지/어머니 선택 클릭 후 화면 전환 안 됨" 재발 수정: `useDeviceProfile.js`의 `setProfileId()`가 `registerDeviceForProfile()` 완료(await)를 기다린 뒤 `localStorage.setItem`/`setProfileIdState`를 실행하던 순서를 바꿔, Firestore 등록 성공 여부와 무관하게 프로필 선택이 항상 즉시 반영되도록 변경(등록은 `.catch(() => {})`로 백그라운드 재시도, 기존 재등록 `useEffect`는 유지). 지시서에 포함됐던 `firestore.rules`의 `isSelfDeviceRegistration()` 단순화는 검토 후 **적용하지 않음** — (1) 에뮬레이터로 직접 확인한 결과 원래 규칙에서도 `arrayUnion()` 자가등록이 정상 동작해 "arrayUnion 평가 실패" 전제가 사실이 아니었고, (2) 단순화하면 기존 보안 테스트 시나리오 3("deviceUids 초기화 시 name 필드 동시 변경 거부")이 깨지며 기기 2대 상한/자기 uid만 추가 가능/타 기기 제거 방지 보호도 함께 사라지는 회귀가 발견되어, 사용자에게 확인 후 제외하기로 결정. `npm run test:rules`(11개 시나리오, `useDeviceProfile.js` 수정만 적용한 상태) 전부 통과, `npm run build` 통과. `npm run deploy` 완료. **배포 관련 예외**: 이 세션 네트워크 정책이 `jungukeu-ctrl.github.io` 접속을 차단(403)해 실사이트 콘솔 확인은 직접 못 함 — 사용자가 실기기(아버지/어머니 폰)에서 프로필 선택 화면 전환을 최종 확인 필요. 타 프로젝트 영향 없음 |
| 2026-07-18 | 운동 화면 Firebase 제거 — localStorage 전용 독립형으로 전환. 기록 보기/설정 탭 숨김. 타 프로젝트 영향 없음. `src/pages/TodayExercise.jsx`를 `useDeviceProfile`(Firestore Anonymous Auth)·`lib/records.js`·`lib/stats.js` 의존 없이 localStorage(`chair-exercise-records` 키, 프로필 선택값은 기존 `chair-exercise-device-profile` 키 그대로 재사용)로 전면 교체. 지시서 원안에서 빠져있던 기존 PC/태블릿 접속 제한 기능(`isMobileDevice()` 안내 문구 + 체크 버튼 비활성화)은 사용자 확인 후 그대로 유지. `src/App.jsx` 내비게이션에서 "기록 보기"/"설정" 링크 제거(라우트는 유지). `npm run build` 통과. PR #24 → main 병합, `npm run deploy` 완료. **배포 관련 예외**: 이 세션 네트워크 정책이 `jungukeu-ctrl.github.io` 접속을 차단(403)해 실사이트 콘솔 확인은 직접 못 함 — 사용자가 실기기에서 프로필 선택/체크/메모/영상 링크/축하 도장 동작 및 새로고침 후 유지 여부를 최종 확인 필요 |
| 2026-07-07 | 운동 카드에 유튜브 영상 링크 추가 + 세션 간 `.env` 재입력 문제 해결(작업지시서 기반, 2개 커밋으로 분리): (1) `src/constants/exercises.js`에 `VIDEO_URL`(`https://youtu.be/C8bN9cKxUlI`) 상수와 각 운동의 `videoTimestampSec`(노션 원본 "동작설명" 타임스탬프, 8개 값 mm:ss 역산 검증 완료) 추가, `ExerciseCard.jsx`에 "▶ 영상 보기" 링크(`?t={초}s`) 추가, `App.css`에 `.video-link` 스타일 추가 — 정적 상수라 체크/메모/진행률/스트릭 로직·Firestore 스키마·대시보드 영향 없음. (2) 이전에 의도적으로 내렸던 "`.env`는 gitignore 유지" 결정을 사용자 확인 후 뒤집음 — Firebase 웹 config(`apiKey` 포함)는 클라이언트 번들에 어차피 노출되는 비밀값 아닌 값이고 보안은 `firestore.rules`가 담당한다는 전제로, `.env_origin`(`chairexercise-bfd03` 프로젝트 값, PLAN.md 기록과 일치 확인) 신규 커밋, `scripts/check-env.mjs`가 `.env` 부재 시 자동 복사하도록 수정, `.env.example`/`CLAUDE.md` A-8에 안내 추가. `VITE_FIREBASE_VAPID_KEY`는 미발급이라 `.env_origin`에 미포함(FCM 작업 시 별도 확인 필요, 기존과 동일). `.env` 삭제 후 `npm run build` 실행해 자동 복사 로그+빌드 통과 확인, `npm run test:rules`(13개 시나리오) 회귀 없음 확인. PR #21 → main 병합, `npm run deploy` 완료. **배포 관련 예외**: 이 세션 네트워크 정책이 `jungukeu-ctrl.github.io` 접속을 차단(403)해 실사이트 콘솔 확인은 직접 못 함 — 사용자가 "영상 보기" 링크 노출·재생 위치를 최종 확인 필요(위 "남은 작업" 참고). 타 프로젝트 영향 없음 |
