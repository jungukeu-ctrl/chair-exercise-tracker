...
- `.env_origin`(저장소에 커밋됨, Firebase 웹 config 값 — 비밀값 아님)이 있으면
  `npm run build` 실행 시 `scripts/check-env.mjs`가 자동으로 `.env`로 복사한다.
  따라서 `.env_origin`이 최신이면 사용자에게 값을 다시 물어볼 필요가 없다.
- `VITE_FIREBASE_VAPID_KEY`는 `.env_origin`에 포함돼 있지 않으므로(발급받지 못함),
  FCM 푸시 알림이 필요한 작업 시에는 여전히 별도 확인이 필요하다.
- `.env_origin` 자체가 없거나 오래된 값으로 보이면(프로젝트 변경 등), 작업을
  진행하기 전에 사용자에게 먼저 알리고 값을 요청한다.
