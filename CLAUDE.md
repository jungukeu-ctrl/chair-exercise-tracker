...
- PLAN.md가 "일반 행동 지침 7번"의 Plan + Checklist 역할을 겸한다
  ...
  - 메뉴별 기능검증 시나리오는 PLAN.md의 "기능검증 시나리오" 섹션에 유지
- 코드 작성 전 반드시 변경 범위, 영향받는 기능, 예상 위험을 먼저 분석·보고하고, 사용자 확인("코드작성") 후에만 진행 (예외 없음)

### A-3. 작업 사이클 (DO → CHECK → 기능검증 → ACT)
- DO: 한 번에 하나만 수정, 요청 외 코드 건드리지 않음, 별도 요청 없는 리팩토링 금지
- CHECK: 수정 위치/내용, 영향받은 기능, 잠재적 오류, 계산 로직(진행률·스트릭 등)은 예시 숫자로 검증
- 기능검증: 변경이 영향을 준 메뉴는 PLAN.md "기능검증 시나리오"의 해당 항목을 실제로 확인한 뒤에만 완료로 보고. 새 메뉴/기능을 추가한 경우 시나리오 항목도 함께 추가
- ACT: 기능검증 결과를 포함해 "코드작업완료" 알림 후 PLAN.md 갱신 (완료 작업 추가/남은 작업 정리/스키마 변경 반영)
