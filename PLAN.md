...
- [ ] `firestore.rules` **재배포 필요** — 2026-07-07 `deviceUidsOf`/`allowedProfiles` 필드 부재 버그 수정판을 아직 배포 못함(서비스 계정 키 폐기 상태라 이번 세션 권한 없음). 새 서비스 계정 키 발급 후 `firebase deploy --only firestore:rules --project chairexercise-bfd03` 필요, 완료 후 이 항목 삭제하고 배포 직후 재폐기(rotate) 권장
